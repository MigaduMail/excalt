defmodule Excalt.Vcard.AddressbookHandler do
  ...
  def handle_event(:start_element, {xml_element, attributes}, {_current_xml_el, addressbooks}) do
    ...
    if String.match?(xml_element, ~r/address-data-type/) do
      ...
      current_addressbook = append_content_type_and_version(current_addressbook, content_type, version)
  ...
  def handle_event(:characters, content, {current_tag, addressbooks}) do
    ...
    addressbooks = update_current_addressbook(current_tag, ~r/displayname/, :name, addressbooks, content)
    addressbooks = update_current_addressbook(current_tag, ~r/addressbook-descriptio/, :description, addressbooks, content)
  ...
  """
  @spec update_current_addressbook(boolean(), atom(), List.t(), String.t()) :: [] | [%Excalt.Vcard.Addressbook{}]
  ...
  def update_current_addressbook(true, _field, addressbooks, content) when length(content) < 0, do: addressbooks
  ...
  def append_content_type_and_version(%Excalt.Vcard.Addressbook{content_types: content_types, versions: versions} = current_addressbook, content_type, version) do
   %{current_addressbook | content_types: [content_type | content_types], versions: [version | versions]}
  end
